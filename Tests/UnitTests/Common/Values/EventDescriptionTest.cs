using domain.Common.Values;

namespace UnitTests.Common.Values;

public class EventDescriptionTest
{
    [Fact]
    public void should_create_an_event_description()
    {
        //given
        var str = "a great title";
        
        var result = new EventDescription(str);
        Assert.NotNull(result);
        Assert.NotNull(result.Value);
        Assert.NotEqual(0, result.Value.Length);
    }
    
    [Fact]
    public void should_throw_when_invalid_constructor_argument_null()
    {
        var exception = Assert.Throws<ArgumentNullException>(()=>new EventDescription(null!));
        Assert.IsType<ArgumentNullException>(exception);
    }
    
    [Fact]
    public void should_create_an_empty_description()
    {
        var str = "";
        
        var result = new EventDescription(str);
        Assert.NotNull(result);
        Assert.NotNull(result.Value);
        Assert.Equal(0, result.Value.Length);
    }
    
    [Fact]
    public void should_throw_when_invalid_constructor_argument_too_long()
    {
        var tooLongString = "";
        var exception = Assert.Throws<ArgumentException>(()=>new EventDescription(tooLongString));
        Assert.IsType<ArgumentException>(exception);
        Assert.Equal("Description cannot be longer than 1024.",exception.Message);
    }
    
    
}